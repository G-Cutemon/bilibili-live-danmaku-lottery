window.biWebSock = (function () {
        ...
        init: function (roomid) {
            ...
            socket.onmessage = function (event) {
                ...
                if (data.op && data.op === 5) {
                    ...
                    for (
                        var offset = 0; offset < dataView.byteLength; offset += packetLen
                    ) {
                        ...
                        var recData = [];
                        for (
                            var i = offset + headerLen; i < offset + packetLen; i++
                        ) {
    ...
    return {
        ...
        start: function (roomid) {
            ...
            $(".danmaku_info")
                .eq(0)
                .prepend('<div style="color: green">获取弹幕中……</div>');
        ...
        disconnect: function (rc) {
            ...
            $(".danmaku_info")
                .eq(0)
                .prepend('<div style="color: indianred">弹幕获取停止……</div>');
            if (this.room) {
                console.log(
                    "%c 正在退出房间：" + this.room.roomid + "...",
                    "color: red"
                );
                if (rc) {
                    console.log(
